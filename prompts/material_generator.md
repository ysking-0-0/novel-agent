...
【朗读断句要求（最高优先级·防 TTS 读错）】
TTS 会按标点断句，标点不当会导致听感错位（如"X被Y"被动句在主语后停顿，听起来像缺主语）。每段 text 必须遵守：
1. 每段 30-70 字，严禁超长段落（>80 字必拆成两段）
2. 短句为主，长句用逗号分层；一句话内不嵌套过多修饰成分
3. 被动句"XX被YY"必须在主语后加逗号：写"XX，被YY"（如"两位巨擘，被这黄口小儿激怒"），严禁"XX被YY"连写（TTS 会在"被"前错误停顿）
4. 每句控制在 20 字以内，句号明确断句；避免"XX和YY和ZZ"多层并列无停顿

...
1. 世界观介绍：
   - 本集涉及的专有名词/设定必须向观众解释（如"十万里大荒""剑门""魔魂阴瘴"是什么）
   - 人族为何弱小、部落为何贫困、剑门为何是圣地等背景信息
   - 首次出现的地名/组织/种族/历史事件都要简要说明其含义和重要性

2. 境界体系介绍：
   - 涉及修炼境界时必须解释（如"炼气士""修灵""魂魄""出窍"分别是什么概念）
   - 境界层次说明（如外门弟子→内门弟子→长老的晋升体系，各境界的能力差异）
   - 修炼原理简述（如魂魄如何壮大、识海是什么、观想图的作用）

3. 人物地位与状况（必须讲清）：
   - 每个出场人物的社会地位（如"外门弟子地位极低，只能学最粗浅的功法"）
   - 当前处境/困境（如"穷困部落出身，买不起灵药""十六岁前修不成灵就被逐出山门"）
   - 人物的动机与决心（如"必须自己找灵药""勤修苦练比人刻苦百倍"）
   - 身体状况/伤势（如"前胸三道爪痕、十几处伤疤——这不是普通少年该有的"）

4. 重要剧情的因果讲解：
   - 不能只复述发生了什么，必须讲清"为什么发生"（因果链）
   - 关键转折点要重点解说（如"为什么魔魂阴瘴提前爆发""为什么钟岳必须独自冒险"）
   - 剧情高潮要营造悬念（如"命悬一线，他将如何脱险"）

5. 伏笔与誓言复述（必须包含）：
   - 本集埋设的伏笔必须向观众点明并解释其潜在意义（如"身上伤疤暗示身世之谜"）
   - 人物立下的誓言/决心必须着重复述（如"我要修成灵，绝不辜负部落期望"）
   - 回收的历史伏笔要说明来龙去脉（如"之前埋的XX伏笔在本集兑现了"）
   - 留下的悬念结尾要吊观众胃口（如"灯中传出的声音究竟来自谁"）

6. 情感与氛围渲染：
   - 用口语化、带入感的讲解风格，像朋友讲故事一样娓娓道来，自然亲切
   - 危险场面营造紧张感，温情场面营造感动，谜团营造神秘
   - 禁止使用说书套话：不要"各位看官""话说""且说""且听我道来""欲知后事如何，且听下回分解"等传统说书词
   - 直接进入内容，自然过渡，不要多余的寒暄或铺垫词

文案总量建议 800-1500 字，分 8-15 段（每段对应一个 tts_meta 段落）。
...
- 参考范例（『生图频率描述.txt』节选）：
  ...
  ——7幅图覆盖约70秒讲解，每图约10秒
- 数量要求（硬性下限）：
  图片张数 = max(语音总时长 / 10秒, tts_meta段数)
  即：图片数必须 ≥ TTS段数，且每10秒至少一张图
  示例：16段TTS约300秒 → 至少30张图；10段TTS约200秒 → 至少20张图
...
【输出格式 严格 JSON】
{
  "script": "整集讲解文案（800-1500字，含世界观/境界/地位状况/因果/伏笔誓言复述，用『』分隔段落，每段对应一个叙事单元，自然口语风格，禁用说书套话）",
  "image_prompts": [
    {
      "index": 1,
...
严格要求：
1. 讲解文案必须包含【讲解文案内容要求】中的全部要素：世界观介绍、境界体系、人物地位与状况、因果讲解、伏笔与誓言复述。不是流水账复述，而是面向观众的剧情解说+背景科普。
